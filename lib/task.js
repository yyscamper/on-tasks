...
function factory(
    ...
    var COMMON_OPTIONS_SCHEMA_NAME = 'common-task-options.json';

    function getCommonSchema() {
        return validator.getTaskSchema(COMMON_OPTIONS_SCHEMA_NAME);
    }

    function getTaskSpecificSchema(definition) {
        if (definition.hasOwnProperty('optionsSchema')) {
            if (_.isString(definition.optionsSchema)) {
                return validator.getTaskSchema(definition.optionsSchema);
            }
            else {
                assert.object(definition.optionsSchema, 'task built-in options schema');
                return definition.optionsSchema;
            }
        }
        //TODO: Remove legacy schema after new design is done
        else if (definition.hasOwnProperty('schemaRef')) {
            return validator.getTaskSchema(definition.schemaRef);
        }
    }

    function getJobSchema(definition) {
        var Job = injector.get(definition.runJob);
        if (Job.hasOwnProperty('hasSchema') && Job.hasSchema()) {
    ...
    function Task(definition, taskOverrides, context) {
        ...
        self.schema = getTaskSpecificSchema(definition);
    ...
    Task.prototype.findJobClass = function() {
        //TODO: Only keep describeJob after task-schema is fully ready
        var jobName = this.definition.runJob;
        if (this.definition.hasOwnProperty('schemaRef') && //let legacy schema works
                !this.definition.hasOwnProperty('optionsSchema')) {
    ...
    Task.prototype.instantiateJob = function() {
        this.job = new this.JobClass(this.options, this.context, this.instanceId);
    };

    Task.prototype.validateSchema = function(options, flags) {
        var self = this;

        try {
            //always validate common task schema
            validator.validateTaskSchema(COMMON_OPTIONS_SCHEMA_NAME, options, flags);

            //validate task-specific schema if existing
            if (self.schema) {
                validator.validateTaskSchema(self.schema, options, flags);
            }

            //validate job schema if existing
            if (self.JobClass.hasOwnProperty('hasSchema') && self.JobClass.hasSchema()) {
                self.JobClass.validateOptions(options);
            }
        }
        catch (err) {
            //add injectableName into error message will help identify problem
            throw new Error(self.definition.injectableName + ': ' + err.message);
        }
    };
    ...
    Task.prototype.run = function() {
        ...
        return Promise.resolve()
            ...
            .then(function() {
                ...
                //ready
                self.validateSchema(self.options, { skipContext: false } );
            })
            .then(function() {
                return self._run();
            })
    ...
    Task.prototype.compile = function compile() {
        ...
        .then(function() {
            //TODO:Remove the judgement and enable the code block after task-schema is fully
            //ready
            self.validateSchema(self.options, { skipContext: true });
        });
    };
    ...
    Task.validateDefinition = function validateDefinition(definition) {
        ...
        var schema = getTaskSpecificSchema(definition);
        if (schema) {
            assert.object(schema, 'task schema');
        }

        if (definition.hasOwnProperty('schemaRef') && !definition.hasOwnProperty('optionsSchema')) {
    ...
    Task.getSchema = function (definition, flags) {
        flags = flags || {};
        var schemas = [];

        if (!flags.excludeCommon) {
           schemas.push(getCommonSchema());
        }

        if (!flags.excludeTaskSpecific) {
            schemas.push(getTaskSpecificSchema(definition));
        }

        if (!flags.excludeJob) {
            schemas.push(getJobSchema(definition));
        }
