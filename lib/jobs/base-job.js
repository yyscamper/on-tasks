...
function baseJobFactory(
    ...
    var jobClassFuncs = {
        defineSchema: function (JobClass, schema) {
            if (_.isObject(schema) || _.isString(schema)) {
                Object.defineProperty(JobClass, 'schema', {
                    value: schema,
                    writable: false, //schema is readonly for each job class
                    enumerable: true,
                    configurable: false
                });
            }
            else {
                throw new Error('schema should only be either object or string');
            }
        },

        hasSchema: function (JobClass) {
            return JobClass.hasOwnProperty('schema');
        },

        getSchema: function (JobClass) {
            var schema = JobClass.schema;
            if (_.isString(schema)) { //schema filename
                schema = validator.getJobSchema(schema);
            }
            return schema;
        },

        validateOptions: function (JobClass, options, flags) {
            //validate the options against schema
            var schema = JobClass.getSchema();
            if (schema) {
                assert.object(schema);
                validator.validateJobSchema(schema, options, flags);
            }
        }
    };

    BaseJob.propagate = function(JobClass) {
        //copy all prototype properties
        util.inherits(JobClass, BaseJob);

        //copy static properties
        _.forOwn(jobClassFuncs,  function(func, name) {
            JobClass[name] = func.bind(null, JobClass);
        });
    };
    ...
    function BaseJob(logger, options, context, taskId) {
        ...
        if (self.constructor.hasOwnProperty('hasSchema') &&
                self.constructor.hasSchema()) {
            self.constructor.validateOptions(options);
        }
