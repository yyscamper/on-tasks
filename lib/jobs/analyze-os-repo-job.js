...
function analyzeOsRepoJobFactory(
    ...
    BaseJob.propagate(AnalyzeOsRepoJob);
    ...
    AnalyzeOsRepoJob.defineSchema({
        properties: {
            osName: {
                description: "The name of target OS",
                enum: ["ESXi"],
                readonly: true
            },
            version: {
            ...
            repo: {
                description: "The OS http repository which contains all data that required for os installation", //jshint ignore: line
                type: "string",
                pattern: "^http://"
            }
        },
        required: ["osName", "version", "repo"]
    });
