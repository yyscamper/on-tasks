...
function taskGraphFactory(
    ...
    TaskGraph.prototype._getBaseTask = function(definition) {
        ...
        if (!definition.hasOwnProperty('implementsTask')) {
            //TODO:
            //make a mock baseTask here to make following code happy, but eventually the baseTask
            //should be removed
            assert.string(definition.runJob,  'The taskDefinition should have runJob if no baseTask');
            return {
                friendlyName: 'fakeBaseTaskFriendlyName',
                injectableName: 'fakeBaseTaskName',
                runJob: definition.runJob,
                requiredOptions: [],
                requiredProperties: {},
                properties: {}
            };
