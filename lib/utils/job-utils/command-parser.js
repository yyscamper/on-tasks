...
function commandParserFactory(Logger, Promise, _) {
    ...
    var ohai = 'sudo ohai --directory /etc/ohai/plugins',
        ...
        driveid = 'sudo ls /dev/disk/by-id/';
    ...
    CommandParser.prototype[driveid] = function(data) {
        ...
        try {
            /*
            1.SATA ID Example under ESXi
            t10.ATA_____SATADOM2DSV_3SE__________________________20150522AA999205008D
            2.SATA ID Example under Linux
            ata-SATADOM-SV_3SE_20150522AA999205008D
            3.SCSI ID Example under ESXi
            naa.6001636001940be01dcdeaba0558f2e0
            4.SCSI ID Example under Linux
            scsi-36001636001940be01dcdeaba0558f2e0
            */
            var lines = data.stdout.split('\n');
            var scsiLines = [], sataLines = [];
            console.log(lines);
            console.log(lines.length);
            for (var i=0; i<lines.length; i++) {
                if ( lines[i] && !(lines[i].match('part'))){
                    if (lines[i].startsWith('scsi')) {
                        scsiLines.push(lines[i]);
                    }
                    else if (lines[i].startsWith('ata')) {
                        sataLines.push(lines[i]);
                    }
                }
            }
            var esxiSata = _.map(sataLines, function(line) {
                var split = line.toUpperCase().split(/-|_/);
                return 't10.' + split[0] + '_____' + split[1] + '2D'
                    + split[2] + '_' + split[3] + '__________________________' + split[4];
            });
            var esxiScsi = _.map(scsiLines, function(line) {
                var split = line.split(/-|_/);
                return 'naa.' + split[1].slice(1);
            });
            var linuxParsed = _.map([sataLines,scsiLines], function(line) {
                return '/dev/disk/by-id/' + line;
            });
            return Promise.resolve({ data: { esxiDriveIds: esxiSata.concat(esxiScsi),
                linuxDriveIds: linuxParsed}, source: 'driveid', store: true });
        } catch (e) {
            return Promise.reject({ source: 'driveid', error: e });
        }
    };
